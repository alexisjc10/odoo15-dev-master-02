...
def load_xml_data(invoice, load_lines, account_id, product_id=False, analytic_account_id=False):
    try:
        invoice_xml = etree.fromstring(base64.b64decode(invoice.xml_comprobante))
    ...
    invoice.name = invoice_xml.xpath("inv:NumeroConsecutivo", namespaces=namespaces)[0].text
    ...
    if receptor_node:
        ...
        receptor = '999999999'
        # raise UserError(_('El receptor no está definido en el xml :' + receptor_node[0].text + ' NUMBER'))

    if emisor != invoice.company_id.vat:
        raise UserError(_('El receptor no corresponde con la compañía actual con identificación ' +
                          receptor + '. Por favor active el cliente correcto.'))
    ...
    partner = invoice.env['res.partner'].search([('vat', '=', receptor), '|',
    ...
    if load_lines:
        ...
        for line in lines:
            ...
            for codigo in codigo_articulo_linea:
                ...
                if codigo_articulo_xml == '079959210813':
                    codigo_articulo_xml = 'COD30479'
                elif codigo_articulo_xml == 'COD13522':
                    codigo_articulo_xml = 'COD08110'
                elif codigo_articulo_xml == 'S00015':
                    codigo_articulo_xml = 'COD02432'
                elif codigo_articulo_xml == 'TCG-044':
                    codigo_articulo_xml = 'COD06320'
                break
            product_odoo = invoice.env['product.product'].search([('old_code', '=', codigo_articulo_xml), ], limit=1)
